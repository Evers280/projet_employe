...
class EmployeForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if drh:
            self.fields['username'] = forms.CharField(max_length=150, required=True, widget=forms.TextInput(attrs={'class': 'input input-primary w-full', 'placeholder': 'Nom d\'utilisateur'}))
    ...
    def save(self, commit=True):
        ...
        if 'username' in self.cleaned_data:
            user = User.objects.create_user(
                username=self.cleaned_data.get('username', self.cleaned_data['email']),
